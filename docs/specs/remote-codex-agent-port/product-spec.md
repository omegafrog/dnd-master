...
- AI 실행 실패가 모험의 게임 상태를 바꾸지 않게 한다.
...
6. 실행이 성공하면 서버는 최종 결과 한 건을 기존 AI Game Master 흐름에 반환하고 해당 요청 ID를 지운다.
7. 실행이 실패하면 서버는 오류를 표시하고 게임 상태를 유지하며 해당 요청 ID를 지운다.
...
- 같은 요청 ID의 성공, 실행 실패, 시간 초과 또는 사용자 PC 에이전트 연결 끊김이 확인되면 진행 중인 요청 ID를 지운다. 그 뒤에만 수동 재시도할 수 있다.
...
- 실행 실패, 시간 초과, 사용자 PC 에이전트 연결 끊김이 발생하면 게임 상태는 바뀌지 않는다.
- 실패 시 Solo Player에게 오류를 표시하고 같은 행동을 수동으로 다시 시도할 수 있게 한다.
- 자동 재시도는 하지 않는다.
...
- 실행 실패, 시간 초과, 연결 끊김은 게임 상태를 유지하고 오류와 수동 재시도 가능 상태를 제공한다.
