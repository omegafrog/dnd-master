...
| --- | --- | --- | --- |
| AI 실행 요청 | Adventure Session | Adventure Session ID | 진행 중 요청 ID 하나 제한 |
...
| 요청 종료 | Adventure Runtime | 같은 ID 조건부 해제와 정상 결과 처리 | 결과 처리 성공 또는 실패 확정 | 정상 결과 처리 실패 시 게임 상태 변경 없음 |
...
| 실패 시 게임 상태 유지 | 정상 결과 처리 전 실패 경로 회귀 테스트 |
