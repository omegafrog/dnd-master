...
지도에 인쇄된 격자와 플레이 격자가 어긋나면 토큰의 위치를 읽기 어렵다. 사용자는 전체 지형을 검수하지 않고 이미 공개된 영역에서 격자 정렬만 수정하고 싶다.
...
포함: 정사각 격자 3단계 맞춤, 확대경, 적용·취소·실패 후 재시도, 재편집, 공개 범위 및 기존 플레이 상태 유지.

제외: 벽·문 편집, 안개 정책 변경, AI 자동 탐지 추가, 강제 전체 검수, 게임 칸 수 변경, 회전·육각 격자·비선형 이미지 보정. 앞선 계획의 추가 기능은 본 명세보다 우선하지 않는다.
