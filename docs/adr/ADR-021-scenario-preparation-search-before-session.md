...
- 시나리오 준비에는 별도 내부 검색 계약을 사용한다. 준비 호출자는 저장된 Scenario Source Bundle에서 소유자와 선택한 원문 문서·추출 버전을 확인한다.
- Document Knowledge는 전달된 문서·버전의 발행 상태와 소유자·허가 범위를 검증하고, 기존 Dense/BM25/RRF 검색 구현을 공유한다.
