...
| [RAG-018](rag-018-server-owned-citation-keys.md) | — | completed | RAG-016 | 모델이 복사한 인용 대신 서버 정본 citation key를 최종 근거로 사용한다. |
| [RAG-019](rag-019-bounded-projection-repair.md) | — | ready-for-agent | RAG-018 | 구조화 projection 위반을 bounded repair·재생성·정직한 실패로 처리한다. |

## Dependency decision

Dependency graph evaluated on 2026-08-26. RAG-014 and RAG-015 were completed, enabling RAG-016 and RAG-017; RAG-018 implementation and independent review are complete, so dependent RAG-019 is promoted to ready-for-agent.
