...
> 상태: 수정 반영됨. 당시 재현 기록은 보존한다. Story Plan timeout은 90초로 제한되고, provider 장애는 안전한 오류와 재시도로 표시된다. 실제 provider 성공/5턴 backend acceptance는 `BACKEND_E2E_ACCEPTANCE=1` 환경에서 재실행한다.
