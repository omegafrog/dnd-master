CREATE TABLE IF NOT EXISTS scenario_source_bundle (
    bundle_id UUID PRIMARY KEY,
    owner_player_id UUID NOT NULL,
    current_revision BIGINT NOT NULL CHECK (current_revision >= 1)
);

CREATE INDEX IF NOT EXISTS scenario_source_bundle_owner_idx
    ON scenario_source_bundle(owner_player_id);

CREATE TABLE IF NOT EXISTS scenario_source_bundle_revision (
...
CREATE TABLE IF NOT EXISTS scenario_source_bundle_revision_document (
