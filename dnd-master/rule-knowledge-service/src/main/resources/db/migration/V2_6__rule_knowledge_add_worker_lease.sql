...
CREATE UNIQUE INDEX rulebook_vector_operation_key_uq
...
ALTER TABLE rulebook_vector_chunk
    ADD COLUMN embedding_next public.vector(1536);

CREATE INDEX rulebook_vector_chunk_embedding_next_hnsw_idx
    ON rulebook_vector_chunk USING hnsw (embedding_next public.vector_cosine_ops)
    WHERE embedding_next IS NOT NULL;

CREATE VIEW rulebook_vector_chunk_transition AS
