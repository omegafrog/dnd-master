...
- **사용자 PC 에이전트**: Solo Player의 PC에서 실행되어 그 PC에만 있는 Codex 로그인 정보를 사용해 서버의 AI 요청을 처리하는 프로그램. Solo Player마다 활성 연결은 하나만 허용한다.
...
- **공간 요소**: 전술 지도의 한 칸 이상에 놓이며 이동·시야·발견·상호작용에 반응하는 지도 상태. 함정, 비밀문, 위험 지역, 상호작용 대상, 마법 범위 효과를 포함하며 캐릭터나 몬스터를 나타내는 토큰과 구분한다.
- **이동 제안**: Solo Player의 직접 토큰 이동 또는 자연어 이동 의도를 실제 지도 경로로 표현한 미확정 후보. 확인 전에는 위치, 전장의 안개, 공간 요소 상태, 모험 진행을 바꾸지 않는다.
- **이동 해결**: 확인된 이동 제안을 경로의 칸 순서대로 처리하고, 시야 공개·공간 요소 반응·적의 인지를 반영해 실제 마지막 위치와 공개 가능한 결과를 확정하는 과정.
- **이동 예약**: 이동 해결에 필요한 룰북 판정이 끝날 때까지 Combat Map이 기준 지도 버전과 진행 위치를 보존하는 내부 작업 상태. 예약 중 공개 지도 상태는 바뀌지 않으며, 최종 반영 전 실패하면 취소하고 모든 판정이 준비되면 위치·공간 요소·전장의 안개를 한 번에 반영한다.
