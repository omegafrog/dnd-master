...
public final class PostgresMovementResolutionOperationRepository implements MovementResolutionOperationRepository {
    ...
    private MovementResolutionOperation write(MovementResolutionOperation value, boolean insert) {
        ...
        try (var connection = dataSource.getConnection(); var statement = connection.prepareStatement(sql)) {
            if (insert) { statement.setObject(1, value.operationId()); statement.setObject(2, value.mapId().value()); statement.setObject(3, value.commandId()); statement.setObject(4, value.playerId().value()); statement.setObject(5, value.tokenId().value()); statement.setString(6, path); statement.setInt(7, value.requestedPath().distance()); statement.setString(8, value.fingerprint()); statement.setLong(9, value.expectedVersion()); bindProgress(statement, 10, value); statement.setLong(22, value.persistenceVersion()); statement.setString(23, value.retryResumeStatus().name()); }
            else { bindProgress(statement, 1, value); statement.setString(13, value.retryResumeStatus().name()); statement.setObject(14, value.operationId()); statement.setLong(15, value.persistenceVersion()); }
