...
public final class PostgresMovementResolutionOperationRepository implements MovementResolutionOperationRepository {
    ...
    private MovementResolutionOperation write(MovementResolutionOperation value, boolean insert) {
        ...
        String sql = insert
                ? "INSERT INTO combat_map_movement_operation(operation_id,map_id,command_id,player_id,token_id,requested_path,path_distance,fingerprint,expected_version,status,cursor,current_x,current_y,traversed_path,result_traversed_path,result_status,result_final_x,result_final_y,result_map_version,result_public_events,result_interruption_reason,result_hostile_token_id,retry_count,operation_version,retry_resume_status,pending_check_id,pending_feature_id,pending_feature_type,pending_trigger,pending_rule_reference,pending_difficulty,pending_mode,pending_dice_expression,pending_modifier,pending_ownership,pending_owner_actor,pending_owner_player_id,check_outcomes,cancel_command_id,hostile_observations) VALUES (" + "?,".repeat(39) + "?)"
                : "UPDATE combat_map_movement_operation SET status=?,cursor=?,current_x=?,current_y=?,traversed_path=?,result_traversed_path=?,result_status=?,result_final_x=?,result_final_y=?,result_map_version=?,result_public_events=?,result_interruption_reason=?,result_hostile_token_id=?,retry_count=?,retry_resume_status=?,pending_check_id=?,pending_feature_id=?,pending_feature_type=?,pending_trigger=?,pending_rule_reference=?,pending_difficulty=?,pending_mode=?,pending_dice_expression=?,pending_modifier=?,pending_ownership=?,pending_owner_actor=?,pending_owner_player_id=?,check_outcomes=?,cancel_command_id=?,hostile_observations=?,operation_version=operation_version+1,updated_at=CURRENT_TIMESTAMP WHERE operation_id=? AND operation_version=?";
        try (var connection = dataSource.getConnection(); var statement = connection.prepareStatement(sql)) {
            if (insert) { statement.setObject(1, value.operationId()); statement.setObject(2, value.mapId().value()); statement.setObject(3, value.commandId()); statement.setObject(4, value.playerId().value()); statement.setObject(5, value.tokenId().value()); statement.setString(6, path); statement.setInt(7, value.requestedPath().distance()); statement.setString(8, value.fingerprint()); statement.setLong(9, value.expectedVersion()); bindProgress(statement, 10, value); statement.setLong(24, value.persistenceVersion()); statement.setString(25, value.retryResumeStatus().name()); bindPending(statement, 26, value); statement.setObject(39, value.cancelCommandId()); statement.setString(40, encodeHostileObservations(value)); }
            else { bindProgress(statement, 1, value); statement.setString(15, value.retryResumeStatus().name()); bindPending(statement, 16, value); statement.setObject(29, value.cancelCommandId()); statement.setString(30, encodeHostileObservations(value)); statement.setObject(31, value.operationId()); statement.setLong(32, value.persistenceVersion()); }
    ...
    private static MovementResolutionOperation read(ResultSet row) throws SQLException {
        ...
        UUID hostileTokenId = row.getObject("result_hostile_token_id", UUID.class);
