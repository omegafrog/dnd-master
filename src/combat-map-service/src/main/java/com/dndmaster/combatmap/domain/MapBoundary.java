...
public record MapBoundary(int x, int y, Orientation orientation, Kind kind, boolean open) {
    ...
    public static MapBoundary parse(String value) {
        ...
        if (parts.length != 4 && parts.length != 5) throw new IllegalArgumentException("map boundary must be x,y,orientation,kind,open");
        try {
            return new MapBoundary(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]),
                    ...
                    Kind.valueOf(parts[3].trim().toUpperCase(Locale.ROOT)), parts.length == 5 && Boolean.parseBoolean(parts[4]));
        } catch (RuntimeException exception) { throw new IllegalArgumentException("invalid map boundary", exception); }
    }

    public String encoded() { return x + "," + y + "," + orientation + "," + kind + "," + open; }
    ...
    public boolean blocks(GridPosition from, GridPosition to) {
        if (kind == Kind.DOOR && open) return false;
