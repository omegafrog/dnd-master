...
public record MovementCheckRequest(UUID checkId, UUID operationId, UUID featureId,
        ...
        com.dndmaster.combatmap.domain.GridPosition targetCell, int cursor) {
    public MovementCheckRequest(UUID checkId, UUID operationId, UUID featureId,
        ...
        this(checkId, operationId, featureId, featureType, trigger, ruleReference,
                "1d20", 0, difficulty, mode, owner, null, -1);
    }
    ...
    public MovementCheckRequest(UUID checkId, UUID operationId, UUID featureId,
            ...
            String diceExpression, int modifier, Integer difficulty, String mode, MovementCheckOwner owner) {
        this(checkId, operationId, featureId, featureType, trigger, ruleReference,
                diceExpression, modifier, difficulty, mode, owner, null, -1);
    }

    public static MovementCheckRequest hostile(UUID checkId, UUID operationId, UUID hostileTokenId,
            SpatialTrigger trigger, String ruleReference, String diceExpression, int modifier, Integer difficulty,
            String mode, MovementCheckOwner owner, com.dndmaster.combatmap.domain.GridPosition targetCell, int cursor) {
        return new MovementCheckRequest(checkId, operationId, hostileTokenId, null, trigger, ruleReference,
                diceExpression, modifier, difficulty, mode, owner, targetCell, cursor);
    }
    ...
    public MovementCheckRequest {
        ...
        if (featureType == null && owner.actor() != MovementCheckActor.ENEMY) {
            throw new IllegalArgumentException("feature type must not be null");
        }
        ...
        if (owner.actor() == MovementCheckActor.ENEMY && targetCell == null) {
            throw new IllegalArgumentException("enemy check target cell must not be null");
        }
    }

    public PendingMovementCheck playerView() {
        if (owner.actor() != MovementCheckActor.PLAYER) return null;
        String label = "지각 판정";
