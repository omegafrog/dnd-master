...
test('tactical map aligns its grid and visibly renders fog, tokens, and legend', async ({ page }) => {
  ...
  expect(styles.cellBorderColor).toBe('rgb(33, 110, 90)')
  expect(Number.parseFloat(styles.cellBorderWidth)).toBe(3)
  expect(styles.cellBackground).toBe('rgba(24, 30, 34, 0.35)')
  expect(styles.cellBackgroundImage).toBe('none')
  await expect(playerCell).toHaveCSS('background-color', 'rgba(24, 30, 34, 0.35)')
