...
import type { AdventureSessionApi, AdventureSessionView, CharacterSheetSummary } from './AdventureSessionApi'

type SessionApi = Pick<AdventureSessionApi, 'read' | 'listOwnedCharacters' | 'copyOwnedCharacter' | 'addMember' | 'removeMember' | 'complete' | 'delete'>
...
export function AdventureSessionPanel({ api, ownerPlayerId, sessionId }: { api: SessionApi; ownerPlayerId: string; sessionId: string }) {
  ...
  const load = () => void Promise.all([api.read(sessionId), api.listOwnedCharacters(ownerPlayerId)]).then(([nextSession, ownedCharacters]) => { setSession(nextSession); setCharacters(ownedCharacters) }).catch(error => setMessage(error instanceof Error ? error.message : '세션을 불러오지 못했습니다.'))
