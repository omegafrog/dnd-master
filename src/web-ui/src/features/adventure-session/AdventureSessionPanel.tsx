...
export function AdventureSessionPanel({ api, ownerPlayerId, sessionId, playApi }: { api: SessionApi; ownerPlayerId: string; sessionId: string; playApi?: AdventurePlayApi }) {
  ...
  async function setControlMode(member: AdventureSessionView['party'][number], controlMode: 'DIRECT' | 'AGENT') {
    if (!session || frozen || updatingMemberId !== null || member.controlMode === controlMode || !api.replaceMember) return
    setUpdatingMemberId(member.characterSheetId)
  ...
  return <section className="session-page" aria-labelledby="session-party-heading">
    ...
    {preparingAdventureId && playApi && <section className="session-map-preparation" aria-label="모험 시작 전 맵 준비"><div className="page-heading"><div><p className="eyebrow">MAP PREPARATION</p><h2>모험 시작 전 맵 준비</h2><p>여백을 먼저 자르고, 격자를 맞춘 뒤 AI 벽과 문 초안을 생성해 확인하고 모험을 시작하세요.</p></div></div><CombatMapView adventureId={preparingAdventureId} api={playApi} preparationMode onPreparationComplete={async () => { if (session.status !== 'STARTING') throw new Error('모험 시작 상태가 바뀌었습니다. 세션을 새로고침한 뒤 다시 시도하세요.'); try { const started = await api.start(sessionId, session.version, preparingAdventureId); if (started.status !== 'STARTED') throw new Error('모험 시작이 아직 완료되지 않았습니다. 잠시 후 다시 시도하세요.'); setSession(started); window.location.hash = `#/sessions/${sessionId}?mode=play` } catch (error) { load(); throw error } }} /></section>}
