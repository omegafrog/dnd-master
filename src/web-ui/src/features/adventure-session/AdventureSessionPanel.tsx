...
export function AdventureSessionPanel({ api, ownerPlayerId, sessionId }: { api: SessionApi; ownerPlayerId: string; sessionId: string }) {
  ...
  return <section className="session-page" aria-labelledby="session-party-heading">
    ...
    {session.status === 'STARTED' && <div><button type="button" disabled={pendingEnd !== null} onClick={() => setPendingEnd('complete')}>세션 완료</button><button type="button" disabled={pendingEnd !== null} onClick={() => setPendingEnd('delete')}>세션 삭제</button></div>}
