export function CharacterSkillSelection({
  ...
  const fixedProficientSkills = proficientSkills.filter(skill => !selectedSkills.includes(skill))
  ...
  return <>
    <fieldset>
      ...
      {skillOptions.map(skill => <label key={skill}>
        <input type="checkbox" checked={selectedSkills.includes(skill)} onChange={() => toggleSkill(skill)} disabled={!selectedSkills.includes(skill) && selectedSkills.length >= skillChoiceCount} />
        {skill}
      </label>)}
    </fieldset>
    {expertiseChoiceCount > 0 && <fieldset>
      ...
      {proficientSkills.map(skill => <label key={skill}>
        <input type="checkbox" checked={selectedExpertise.includes(skill)} onChange={() => toggleExpertise(skill)} disabled={!selectedExpertise.includes(skill) && selectedExpertise.length >= expertiseChoiceCount} />
        {skill}
      </label>)}
    </fieldset>}
  </>
