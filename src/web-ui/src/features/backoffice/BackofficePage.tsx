...
type Endpoint = { id: string; name: string; provider: 'OLLAMA' | 'OPENAI_COMPATIBLE' | 'CODEX_CLI'; baseUrl: string; model: string; secretEnvironmentVariable?: string | null; active: boolean }
...
export function BackofficePage({ session }: { session: IdentitySession }) {
  ...
  async function health(id: string) { const response = await fetch(`/api/v1/backoffice/agent-endpoints/${id}/health`, { method: 'POST', headers: headers(session) }); const result = await response.json(); setMessage(result.healthy ? `연결 정상${result.statusCode ? ` (HTTP ${result.statusCode})` : ''}` : `연결 실패: ${result.detail}`) }
  return <section className="setup-page"><div className="page-heading"><div><p className="eyebrow">INTERNAL BACKOFFICE</p><h1>룰북 · Agent 관리</h1></div></div><p role="status">{message}</p>
    ...
    <section className="setup-panel"><h2>Agent endpoint</h2><form onSubmit={saveEndpoint}><input name="name" placeholder="이름" required /><select name="provider"><option value="OLLAMA">Ollama</option><option value="OPENAI_COMPATIBLE">OpenAI 호환</option><option value="CODEX_CLI">Codex OAuth (로컬 CLI)</option></select><input name="baseUrl" type="url" placeholder="http://127.0.0.1:11434 (Codex는 형식용)" required /><input name="model" placeholder="model (예: gpt-5.4-mini)" required /><input name="secretEnvironmentVariable" placeholder="OPENAI_API_KEY (환경변수명만)" /><button>활성 endpoint 저장</button></form><ul>{endpoints.map(item => <li key={item.id}>{item.name} · {item.provider} · {item.model} {item.active ? '· 활성' : ''} <button onClick={() => void health(item.id)}>Health check</button></li>)}</ul></section>
  </section>
}
