...
type Catalog = { catalogRevisionId: string; edition: 'DND_5E_2014' | 'DND_5E_2024'; displayName: string; rulebookId?: string | null; revisionNumber: number; status: string; processingStatus?: string | null; published: boolean }
...
export function BackofficePage({ session }: { session: IdentitySession }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const catalogResponse = await fetch('/api/v1/backoffice/rulebook-catalog', { headers: headers(session) })
