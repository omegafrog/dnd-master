...
function SessionsTab({ knowledgeSet, title }: { knowledgeSet: SessionKnowledgeSet | null; title: string }) {
  return <div className="workspace-tab-panel"><div className="tab-panel-heading"><div><p className="eyebrow">진행 기록</p><h2>이 모험의 세션</h2><p>같은 모험의 준비와 플레이 기록을 이어갑니다.</p></div></div>{knowledgeSet ? <ul className="session-history-list"><li><span className="session-history-icon" aria-hidden="true"><ScrollText size={18} /></span><span><strong>세션 기록</strong><small>{title} · 자료 {knowledgeSet.knowledgeDocumentIds.length}개</small></span><span className="session-history-status">이어갈 수 있음</span><Button variant="ghost" onClick={() => { window.location.hash = `#/sessions/${knowledgeSet.sessionId}?mode=play` }}>이어하기<ChevronRight size={15} aria-hidden="true" /></Button></li></ul> : <EmptyState icon={<CalendarDays size={20} />} title="세션 기록이 없습니다" description="세션 시작을 눌러 첫 기록을 만드세요." />}</div>
}
...
function SessionStartDialog({ open, onOpenChange, title, knowledgeSet, onStart }: { open: boolean; onOpenChange: (open: boolean) => void; title: string; knowledgeSet: SessionKnowledgeSet | null; onStart: (sessionId: string) => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><p className="eyebrow">세션 시작</p><DialogTitle>{title}</DialogTitle><DialogDescription>참가 캐릭터와 이전 진행 상황을 확인한 뒤 플레이 화면으로 이동합니다.</DialogDescription></DialogHeader><div className="session-start-choice"><div className="session-start-option session-start-option-active"><span className="session-start-option-icon"><ScrollText size={18} aria-hidden="true" /></span><span><strong>준비 중인 세션 이어하기</strong><small>{knowledgeSet ? '이전에 저장한 자료와 파티 설정을 이어갑니다.' : '연결된 준비 세션을 찾는 중입니다.'}</small></span><Check size={17} aria-hidden="true" /></div><div className="session-start-option session-start-option-disabled"><span className="session-start-option-icon"><Plus size={18} aria-hidden="true" /></span><span><strong>새 세션</strong><small>새 세션 만들기는 모험 자료 설정에서 진행합니다.</small></span></div></div><DialogFooter><Button variant="ghost" onClick={() => onOpenChange(false)}>취소</Button><Button disabled={!knowledgeSet} onClick={() => knowledgeSet && onStart(knowledgeSet.sessionId)}><Play size={15} aria-hidden="true" />게임 시작</Button></DialogFooter></DialogContent></Dialog>
}
