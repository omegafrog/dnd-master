...
export function SessionRuntime({ adventureId, adventureApi, expectedVersion, playApi, combatSnapshot, mapRefreshToken, onTurnCommitted, adventureTitle, sessionLabel, initialScene, partyCharacters = [] }: { adventureId: string; adventureApi: AdventureApi; expectedVersion?: number | null; playApi: AdventurePlayApi; combatSnapshot?: CombatSnapshot | null; mapRefreshToken?: number; onTurnCommitted?: () => void; adventureTitle?: string; sessionLabel?: string; initialScene?: string | null; partyCharacters?: RuntimePartyCharacter[] }) {
  ...
  return <section className="session-runtime" aria-labelledby="session-runtime-title">
    ...
    <div className="session-runtime-columns">
      <PartyPanel snapshot={combatSnapshot} currentParticipant={current?.displayName} characters={partyCharacters} />
      ...
      <aside className="runtime-context-panel" aria-label="현재 상황">
        <ContextPanel initialScene={initialScene} combatSnapshot={combatSnapshot} mapOpen={mapOpen} setMapOpen={setMapOpen} noteOpen={noteOpen} setNoteOpen={setNoteOpen} map={mapOpen ? <CombatMapView adventureId={adventureId} api={playApi} refreshToken={mapRefreshToken} compact /> : null} />
      </aside>
    </div>
  </section>
}

function PartyPanel({ snapshot, currentParticipant, characters }: { snapshot?: CombatSnapshot | null; currentParticipant?: string; characters: RuntimePartyCharacter[] }) {
...
function ContextPanel({ initialScene, combatSnapshot, mapOpen, setMapOpen, noteOpen, setNoteOpen, map }: { initialScene?: string | null; combatSnapshot?: CombatSnapshot | null; mapOpen: boolean; setMapOpen: (value: boolean) => void; noteOpen: boolean; setNoteOpen: (value: boolean) => void; map: ReactNode }) {
  return <>
