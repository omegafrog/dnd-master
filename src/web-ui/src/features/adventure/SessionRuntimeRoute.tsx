...
export function SessionRuntimeRoute({ sessionId, sessionApi, adventureApi, playApi }: { sessionId: string; sessionApi: AdventureSessionApi; adventureApi: AdventureApi; playApi: AdventurePlayApi }) {
  ...
  if (!session) return <section className="workspace-page workspace-state" aria-busy="true"><p className="eyebrow">세션 실행</p><h1>{message ? '세션을 열 수 없습니다' : '세션을 불러오는 중입니다'}</h1><p role={message ? 'alert' : 'status'}>{message || '잠시만 기다려 주세요.'}</p></section>
  if (!session.adventureId) return <section className="workspace-page workspace-state workspace-state-error"><p className="eyebrow">세션 실행</p><h1>아직 시작되지 않은 세션입니다</h1><p>파티와 자료를 확인한 뒤 세션을 시작하세요.</p><a className="text-link" href={`#/sessions/${encodeURIComponent(sessionId)}`}>준비 화면으로 돌아가기</a></section>
  return <SessionRuntime adventureId={session.adventureId} adventureApi={adventureApi} expectedVersion={undefined} playApi={playApi} />
}
