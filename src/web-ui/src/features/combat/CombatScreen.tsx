...
export function CombatScreen({ snapshot, api, map, onCommandCommitted }: { snapshot: CombatSnapshot; api?: CombatApi; map?: ReactNode; onCommandCommitted?: () => void }) {
  ...
  return <section className="combat-screen" aria-labelledby="combat-title">
    <header className="combat-command-header"><div><p className="eyebrow">전투 진행</p><h1 id="combat-title">전투 · {snapshot.round}라운드</h1></div><p className="combat-current-turn" role="status"><span>현재 차례</span><strong>{current?.displayName ?? '알 수 없음'}</strong><small>{current?.controller === 'PLAYER' ? '직접 조작 · 행동을 선택하세요.' : 'AI 조작 · 행동을 처리하는 중입니다.'}</small></p></header>
...
function InitiativeEntry({ item, current, playerNumber }: { item: CombatParticipant; current: boolean; playerNumber: number }) {
  ...
  return <li className={`${item.controller === 'PLAYER' ? 'ally' : 'enemy'}${current ? ' current' : ''}`} aria-current={current ? 'step' : undefined}><span className="combat-token" aria-hidden="true">{item.controller === 'PLAYER' ? '●' : '▲'}</span><span className="combat-participant-name">{displayName}<small>{item.controller === 'PLAYER' ? '직접 조작' : 'AI 조작'}{item.publicCondition ? ` · ${item.publicCondition === 'healthy' ? '정상' : item.publicCondition === 'enemy' ? '적' : item.publicCondition}` : ''}</small></span><strong>{item.initiative}</strong></li>
}
