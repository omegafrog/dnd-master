...
public final class PostgresCharacterSheetRepository implements CharacterSheetRepository {
    ...
    private void recordHistory(Connection connection, CharacterSheet sheet, UUID operationKey, String operationFingerprint)
        ...
        String sql = "INSERT INTO " + HISTORY_TABLE
                + " (command_id, character_sheet_id, adventure_id, session_id, edition, character_name, character_level, inspiration, race, character_class, background, starting_abilities, operation_key, operation_fingerprint, version)"
                + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            statement.setString(5, sheet.edition().name());
            statement.setString(6, sheet.data().characterName());
            statement.setInt(7, sheet.data().level());
            ...
            statement.setBoolean(8, inspiration);
            statement.setString(9, sheet.data().race());
            statement.setString(10, sheet.data().characterClass());
            statement.setString(11, sheet.data().background());
            statement.setString(12, sheet.data().startingAbilities());
            statement.setObject(13, operationKey);
            statement.setString(14, operationFingerprint);
            statement.setLong(15, sheet.version());
            statement.executeUpdate();
        }
    }
    ...
    private static VersionedCharacterSheet map(ResultSet row) throws SQLException {
        ...
        CharacterSheet sheet = new CharacterSheet(
                ...
                new AdventureId(row.getObject("adventure_id", UUID.class)), new SessionId(row.getObject("session_id", UUID.class)), edition, data,
                row.getLong("version"),
