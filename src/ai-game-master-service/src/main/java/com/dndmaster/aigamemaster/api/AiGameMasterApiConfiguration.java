...
public class AiGameMasterApiConfiguration {
            ...
            try {
                MapEntryPlacementModelPort.EntryPlacementOutput output = java.util.concurrent.CompletableFuture.supplyAsync(() -> adapter.complete(
                    input.soloPlayerId(), "map-entry-placement-" + UUID.randomUUID(),
                            ...
                            + "FIRST_NARRATION=" + input.firstNarration() + "\n"
                            + "PLAYER_ACTION=" + input.action() + "\n"
                            + "GM_JUDGMENT=" + input.judgment() + "\n"
                            + "GM_NARRATION=" + input.narration() + "\n"
                            + "MAP_DATA=" + input.mapData() + "\n"
                            + "MAP_IMAGE=" + (input.imageDataUri().isBlank() ? "not provided" : "provided; inspect the attached image") + "\n"
                            + "TASK=Interpret the completed entry transition and find the visual destination point on the supplied map image. The player has already completed the action. For DESCEND_STAIRS find the first destination-floor point immediately beyond the bottom end of the visible stairs. For a door crossing find the first floor point immediately inside the doorway. Do not calculate a grid cell, count rows or columns, infer a map side, or choose the room center. The server converts your image point to the confirmed grid.\n"
                            + "OUTPUT_CONTRACT=Return exactly one JSON object with status, entryInterpretation, candidates, and reason. status must be RESOLVED, AMBIGUOUS, or UNRESOLVED. entryInterpretation must contain transition, targetScene, anchor, placementRelation, and evidence. candidates must be an array of at most three ranked objects with exitPoint, confidence, source, anchor, reason, and evidence. exitPoint must contain xNormalized and yNormalized: decimal image coordinates from 0 to 1, where 0,0 is the supplied image's top-left edge and 1,1 is its bottom-right edge. Never return grid indices, pixels, rows, columns, x/y fields, or map measurements. evidence must contain concrete quoted narrative facts and a description of the matching map feature. Return UNRESOLVED with an empty candidates array only when the action does not describe a completed map entry or no entry anchor can be localized in the supplied image. If the anchor is visible but several destination points are possible, return them ranked instead of returning an empty array.\n"
    ...
    @Bean
    MovementPlacementModelPort aiGameMasterMovementPlacementModelPort(GmCompletionAdapter adapter,
            com.fasterxml.jackson.databind.ObjectMapper mapper) {
        return context -> {
            try {
    ...
    private static MapEntryPlacementModelPort.EntryPlacementOutput parseEntryPlacement(
        ...
        try {
            ...
            if (root.path("candidates").isArray()) for (var node : root.path("candidates")) {
                ...
                var exitPoint = node.path("exitPoint");
                candidates.add(new MapEntryPlacementModelPort.Candidate(new MapEntryPlacementModelPort.NormalizedPoint(
                        exitPoint.path("xNormalized").asDouble(Double.NaN), exitPoint.path("yNormalized").asDouble(Double.NaN)),
