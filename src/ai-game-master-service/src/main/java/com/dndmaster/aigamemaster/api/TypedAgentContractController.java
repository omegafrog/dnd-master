...
public final class TypedAgentContractController {
    ...
    RuntimeTurnResponse runtimeTurn(
        ...
        return adapter.completeWithSelection(request.soloPlayerId(), request.operationKey(), new com.dndmaster.aigamemaster.infrastructure.ai.GmPrompt(
                        ...
                        + "The map-entry flag is only a proposal. Do not infer, authorize, or describe a grid position from Storybook text; the Adventure Runtime and Combat Map server validate the prepared map binding, map geometry, passable cells, occupancy, and final starting position from map data. A map entry or tactical movement must not be represented as a narrative scene transition solely because the scene label changes. "
