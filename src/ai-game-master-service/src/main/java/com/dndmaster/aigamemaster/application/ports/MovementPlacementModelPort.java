...
/** 공개된 지도 정보만으로 자연어 이동 목적지 후보를 만드는 무상태 계약. */
public interface MovementPlacementModelPort {
    ...
    record MovementPlacementContext(String sourceText, String publicMap, String currentPosition, String tacticalContext) {
        public MovementPlacementContext {
            if (sourceText == null || sourceText.isBlank()) throw new IllegalArgumentException("source text is required");
            publicMap = clean(publicMap); currentPosition = clean(currentPosition); tacticalContext = clean(tacticalContext);
        }
    ...
    record MovementPlacementProposal(String status, Position destination, List<Candidate> candidates, String playerMessage) {
        public MovementPlacementProposal {
            ...
            if (!status.equals("RESOLVED") && !status.equals("AMBIGUOUS") && !status.equals("UNRESOLVED")) throw new IllegalArgumentException("invalid movement placement status");
    ...
    record Candidate(Position destination, double confidence, String reason) {
        public Candidate {
            destination = Objects.requireNonNull(destination, "candidate destination is required");
            if (!Double.isFinite(confidence) || confidence < 0 || confidence > 1) throw new IllegalArgumentException("invalid candidate confidence");
            reason = reason == null ? "" : reason.trim();
        }
    }

    record Position(int x, int y) {
        public Position { if (x < 0 || y < 0) throw new IllegalArgumentException("movement position must not be negative"); }
    }
}
