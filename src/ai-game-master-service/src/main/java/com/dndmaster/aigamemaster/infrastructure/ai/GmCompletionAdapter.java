...
public interface GmCompletionAdapter {
    ...
    default <T> GmCandidateLifecycleResult<T> completeWithOneRepair(
        ...
        try {
            return new GmCandidateLifecycleResult<>(completeWithSelection(soloPlayerId, operationId, prompt, capturingParser, requested), 1);
        } catch (ProviderMalformedResponseException malformed) {
            return new GmCandidateLifecycleResult<>(completeWithSelection(soloPlayerId, operationId + ":repair",
    ...
    default <T> GmCandidateLifecycleResult<T> completeWithOneRepair(
        ...
        return completeWithOneRepair(soloPlayerId, operationId, prompt, repairPrompt, contract.parser(), requested);
    }
