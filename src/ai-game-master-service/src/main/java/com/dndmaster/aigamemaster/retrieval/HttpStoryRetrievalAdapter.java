...
public final class HttpStoryRetrievalAdapter implements RetrievalEvaluationPort {
    ...
    public HttpStoryRetrievalAdapter(String baseUrl, ObjectMapper mapper) { this.client = RestClient.builder().baseUrl(baseUrl).requestFactory(new JdkClientHttpRequestFactory(HttpClient.newBuilder().version(HttpClient.Version.HTTP_1_1).build())).build(); }
