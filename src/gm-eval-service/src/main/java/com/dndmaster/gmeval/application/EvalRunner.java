...
public final class EvalRunner {
    ...
    public EvalRunReport run(List<EvalCase> cases, EvalRunConfiguration config, ResponseGeneratorPort generator, Map<String, PairwiseResponse> pairwiseResponses) {
        ...
        for (EvalCase c : cases) { GeneratedResponse generated; try { generated = generator.generate(c, config); } catch (RuntimeException failure) { reports.add(new EvalRunReport.CaseReport(c.caseId(), null, new EvalResult(List.of(), List.of(), "generator failure"), null, "", failure.getMessage())); continue; } EvalResult result = absolute.evaluate(c, generated.response());
