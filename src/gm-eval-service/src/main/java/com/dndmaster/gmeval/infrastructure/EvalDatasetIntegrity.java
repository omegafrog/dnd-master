...
public final class EvalDatasetIntegrity {
    ...
    public static void validateSeed(List<EvalCase> cases) {
        ...
        for (String required : List.of("rule", "information", "state", "agency", "continuity", "quality")) if (counts.entrySet().stream().filter(e -> e.getKey().contains(required)).mapToInt(Map.Entry::getValue).sum() < 8) throw new IllegalArgumentException("insufficient seed category coverage: " + required);
    }
}
