        ...
        if (stage.combatRequirement() == CombatRequirement.REQUIRED) {
            for (int index = 0; index < stage.combatSkeleton().participants().size(); index++) {
                ...
                for (String key : participant.citationKeys()) {
                    ...
                    boolean participantIdSupported = sourcePresent && supports(sources.get(key).quote(), participant.participantId());
                    boolean entitySupported = nameSupported || participantIdSupported;
                    if (!evidencePresent || !sourcePresent || !entitySupported) {
                        LOGGER.warn("story_plan_combat_participant_grounding_failed stage={} participantId={} role={} name={} citationKeys={} failedCitationKey={} evidencePresent={} sourcePresent={} nameSupported={} participantIdSupported={}",
                                stage.position(), participant.participantId(), participant.role(), participant.name(),
                                participant.citationKeys(), key, evidencePresent, sourcePresent, nameSupported, participantIdSupported);
