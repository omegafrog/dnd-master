...
public final class CrossContextHttpCharacterSheetOwnershipGateway implements CharacterSheetOwnershipPort {
    ...
    public CrossContextHttpCharacterSheetOwnershipGateway(HttpClient client, URI baseUri, Duration timeout, ObjectMapper mapper) { this.client = client; this.baseUri = baseUri; this.timeout = timeout; this.mapper = mapper; }
    public void verify(SessionId sessionId, OwnerPlayerId ownerPlayerId, CharacterSheetId sheetId) {
        try {
            var request = HttpRequest.newBuilder(baseUri.resolve("internal/v1/adventure-sessions/" + sessionId.value() + "/character-sheets/" + sheetId.value() + "/ownership")).timeout(timeout).header("X-Internal-Service", "adventure").GET().build();
