...
public final class PostgresAppliedRuleSetRepository implements AppliedRuleSetRepository {
    ...
    @Override public Optional<AppliedRuleSet> findById(RuleSetId id) {
        String sql = "SELECT rule_set_id, edition FROM applied_rule_set WHERE rule_set_id = ?";
        try (var connection = dataSource.getConnection(); var statement = connection.prepareStatement(sql)) {
            ...
            try (var children = connection.prepareStatement("SELECT rulebook_id FROM applied_rule_set_rulebook WHERE rule_set_id = ? ORDER BY rulebook_id")) {
                ...
                while (childRows.next()) rulebookIds.add(new RegisteredRulebookReference(new RulebookId(childRows.getObject(1, java.util.UUID.class))));
            }
            return Optional.of(new AppliedRuleSet(id, new DndEdition(rows.getString("edition")), new SelectedRulebooks(rulebookIds)));
    ...
    @Override public void save(AppliedRuleSet ruleSet) {
        try (var connection = dataSource.getConnection()) {
            ...
            try (var parent = connection.prepareStatement("INSERT INTO applied_rule_set (rule_set_id, edition) VALUES (?, ?)")) {
                parent.setObject(1, ruleSet.id().value()); parent.setString(2, ruleSet.edition().value()); parent.executeUpdate();
            }
